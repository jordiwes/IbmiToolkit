 ...
 * @package ToolkitApi
 */
class OdbcTransport implements TransportInterface
{
    private $last_errorcode;
    private $last_errormsg;
    public static $transportType = 'odbc';
    ...
    public function connect($database, $user, $password, $options = null)
    {
        if ($options) {
            if ((isset($options['persistent'])) && $options['persistent']) {
                $this->conn = odbc_pconnect($database, $user, $password);
            } else {
                $this->conn = odbc_connect($database, $user, $password);
            }
